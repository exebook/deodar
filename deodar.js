...
/*
Срочно
	при control-O если щёлкнуть в окно терминала, то фокус из правой панели попадает в левую, чего не должно происходить.
	скачок должен обновляться после F4
	...
	После нажатия Энтер в F8 надо сначала закрыть окно(или убрать кнопку, сменить текст), а потом удалять

Подумать
	Сделать кнопку в правке, как в панели для скрытия точек, только скрывать маркеры, и тогда можно редактировать стили, цвета, маркдауны/апы.
	cntrl-F9 (alt-x) пока не отпустишь контрол(или альт) не закрывать вывод
	...
	в правке можно сделать КОЛОНКИ! набрал [|]TAB,пишутся в файл друг за другом
	В правке после вставки можно первое нажатие влево прыгать указателем в начало только что вставленного блока! Необычно, но очень чутко.
	Фолдинг можно сделать и для строк, особенно для длинных.
...
Ашипки
	...
	Если из одного окна правки через вожатый открыть другое, а потом выйти из него, иногда, крайне редко, попадаешь в первое.
...
Идеи
	можно добавлять в список файлов еще один с надписью hidden/скрытые при нажатие на который откроются скрытые файлы, похоже на control+. можно количество скрытых на этом элементе указать.
	показывать закорючку где-то внизу/вверху панели файлов при наличии скрытых файлов
	отображать состояние точки на ободке
	в правке utf8 bom сделать видимым и по табу вводимым
...
Сделано
	сортировать в вожатом по количеству сохранений и времени сохранения
	подставлять из скачка в вожатого части длинных имен, тоже с ободком правщика и ободком списка файлов
...
var enterRule = [ // НАДО: переделать в объект
 { ext: 'deo', deodar: true },
 { ext: 'asm', tty: 'fasm' },
...
// { ext: 'jpg', spawn: ['gpicview', 'xdg-open'] },
 { ext: 'JPG', spawn: 'xdg-open' },
...
setTimeout(function() {
	// сопоставить расширениям редкие программы если они есть на компе.
	sysexe('which', ['gpicview'], function (a) {
		if (a[0].indexOf('gpicview') >= 0) {
			for (var i = 0; i < enterRule.length; i++) {
				if (enterRule[i].ext == 'jpg') {
					enterRule[i].spawn = a[0]
				}
			}
		}
	})
...
//	dnaof(this, fontPath, 40, TController, 24*2,12)
	dnaof(this, fontPath, 12, TController, 58*2,63)
//	44*2,52)
	//37*2,40)
